...
q = """select * from stu_req_detail where status=" " """
cur.execute(q)
rec = cur.fetchall()
print("""
<html>
...
<div class="col-md-7">
<center>
<p style="color:red;"><b>CERTIFICATE REQUEST DETAILS</b></p>
</center>
...
for i in rec:
    ...
    print("""<tr>
        <form method="post">
        <td><input type="text" name="certificate" value="%s" readonly style='width:25px; border:none;'></td>
       <td>%s</td> 
       <td>%s</td>
       <td>%s</td>
       <td>%s</td>
...
if papprove!=None:
    q="""update stu_req_detail set status='Approved' where stu_id='%s' """ %(pcertificate)
    cur.execute(q)
    conn.commit()
