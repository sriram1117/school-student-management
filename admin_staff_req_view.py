...
q = """select * from staff_leave_req_detail where status=" " """
cur.execute(q)
rec = cur.fetchall()
print("""
<html>
...
<div class="col-md-7">
<center>
<p style="color:red;"><b>STAFF LEAVE REQUEST</b></p>
</center>
...
for i in rec:
    print("""<tr>
        <form method="post">
        <td><input type="text" name="certificate" value="%s" readonly style='width:40px; border:none;'></td>
        <td>%s</td> 
        <td>%s</td>
        <td>%s</td>
        <td>%s</td>
        ...
        <td>
        <input class= "btn btn-success" type="submit" name="approve" value="Approve"><br><br>
...
papprove = form.getvalue("approve")
preject = form.getvalue("reject")
pcertificate = form.getvalue("certificate")
pstuid = form.getvalue("stuid")

if papprove != None:
    q = """update staff_leave_req_detail set status='Approved' where id='%s' """ % (pcertificate)
    cur.execute(q)
    conn.commit()
