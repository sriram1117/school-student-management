...
for i in rec:
    ...
    print("""
    <div>
    <form  style="margin-top:-40px;"name="loginform"
        ...
        <div >
            ...
            """ % (stfid, name, date, department, contact, mailid, address, city, state, uname, pwd, salary))
print("""
</table>
</div>
</body>
...
if psubmit != None:
    stfid = form.getvalue("stfid")
    name = form.getvalue("name")
    date = form.getvalue("dob")
    department = form.getvalue("role")
    contact = form.getvalue("phone")
    mailid = form.getvalue("mail")
    address = form.getvalue("address")
    city = form.getvalue("city")
    state = form.getvalue("state")
    salary = form.getvalue("num")
    uname = form.getvalue("uname")
    pwd = form.getvalue("pwd")
    q = """update staff_detail set name='%s', dob='%s' ,department='%s' ,contact='%s', emailid='%s', address='%s' ,city='%s' ,state='%s' ,salary='%s', username='%s', password='%s' where staff_id='%s' """ % (
    name, date, department, contact, mailid, address, city, state, salary, uname, pwd, stfid)
    cur.execute(q)
    conn.commit()
    if (q):
        print("""
        <script>
           ...
           location.href="./admin_staff_view.py";
        </script>
        """)
    else:
        print("""
                <script>
                   ...
                   location.href="./admin_staff_update.py?id=%s";
                </script>
                """ % (pid))
conn.close()