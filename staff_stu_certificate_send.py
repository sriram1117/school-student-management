...
q = """select * from stu_req_detail where id="%s" """ % (pid)
cur.execute(q)
res = cur.fetchall()
...
for i in res:
    ...
    print("""
        <div class="container">
            <div class="leave-form">
                <h2 class="text-center mb-4">CERTIFICATE FORM</h2>
    ...
    print("""
            ...
            <div class="form-group">
                <input type="text" class="form-control"  name="certificate" value="%s">
            
        """ %(cert_type))
    print("""
            </div>
            <div class="form-group">
                <input type="email" class="form-control" name="email" value="%s">
        """ %(mail_id))
    print("""
            </div>
            <div class="form-group">
                <input type="file" class="form-control" name="file" value="%s">
            </div>
            <center>
...
pcertificate=form.getvalue("certificate")
...
if psubmit !=None:
    if len(form)!=0:
        ...
        if pfile.filename:
            ...
            open("Media/" + fn, "wb").write(pfile.file.read())
            q = """insert into certificate_detail (name,certificate,email,image) values('%s','%s','%s','%s')""" % (pname,pcertificate,pemail,fn)
            cur.execute(q)
            conn.commit()
            print("""
                <script>
                alert("Data inserted successfully")
                location.href="./staff_certificate_req_view.py";
                </script>
            """)
            ...
            msg['Form'] = fromaddr
            msg['To'] = toaddr
            msg['Subject'] = "Regarding {}".format(pcertificate)
            body = "Dear {}".format(pname)
